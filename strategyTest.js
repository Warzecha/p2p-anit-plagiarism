...
const {ValidationFacade} = require("./src/validator/ValidationFacade");


let validationFacade = new ValidationFacade(BingSearchStrategy);
...
const iterate = async () => {
    ...
    for (let i = 0; i < praca.length - 5; i++) {
        ...
        let res = await validationFacade.validate(arr, ['rozpoznawanie mowy pdf']);
        if (!!res && res.length > 0) {
            suspicious.push(res)
        }
    }
